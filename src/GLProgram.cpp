...
void GLProgramManager::CompileShader(GLEnumType shaderType, const char * shaderSource)
{
	...
	if (!success) {
		unique_ptr<char[]> charBuffer{new char[this->__glInfoLogLength]};
...
GLint GLProgramManager::GetUniformLocation(const char *name) const {
	return glGetUniformLocation(this->GetProgram(), name);
}

void GLProgramManager::CompileShaderFromFile(GLEnumType shaderType, const char* shaderFileName) {
	auto source = move(GetTextResource(shaderFileName));
	this->CompileShader(shaderType, source.c_str());
}
